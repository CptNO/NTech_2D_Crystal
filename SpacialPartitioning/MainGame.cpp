...
void MainGame::init() {
    ...
    m_spriteFont = std::make_unique<NTCrystal::SpriteFont>("Fonts/chintzy.ttf", 40);
...
struct BallSpawn {
    BallSpawn(const NTCrystal::ColorRGBA8& colr,
...
#define ADD_BALL(p, ...) \
    ...
    ADD_BALL(1.0f, NTCrystal::ColorRGBA8(255, 255, 255, 255),
             1.0f, 1.0f, 0.1f, 7.0f, totalProbability);
    ADD_BALL(10.0f, NTCrystal::ColorRGBA8(0, 0, 255, 255),
             2.0f, 2.0f, 0.1f, 3.0f, totalProbability);
    ADD_BALL(1.0f, NTCrystal::ColorRGBA8(255, 0, 0, 255),
    ...
    for (int i = 0; i < NUM_BALLS; i++) {
        ...
        m_balls.emplace_back(ballToSpawn->radius, ballToSpawn->mass, pos, direction * ballToSpawn->randSpeed(randomEngine),
                             NTCrystal::ResourceManager::getTexture("Textures/circle.png").id,
                             ballToSpawn->color);
    ...
    while (SDL_PollEvent(&evnt)) {
        switch (evnt.type) {
            ...
            case SDL_MOUSEMOTION:
                ...
                m_inputManager.setMouseCords((float)evnt.motion.x, (float)evnt.motion.y);
                break;
            case SDL_KEYDOWN:
